...
su image encoder (Hiera + cuello FPN) como extractor de features congelado, se
descartan la memory attention y el mask decoder original, y se entrena una cabeza
nueva que fusiona los tres niveles del FPN y produce 7 logits por pixel.
...
def build_encoder(backbone='small', device='cpu'):
...
class SAM2Segmenter(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES, backbone='small', dim=128,
                 ...
                 unfreeze_neck=False, device='cpu'):
        super().__init__()
        ...
        self.encoder = build_encoder(backbone, device=device)
        if freeze_encoder:
...
def save_model(model, path='model.th'):
    ...
    if not model.encoder_congelado:
        ...
        estado['encoder_parcial'] = {k.replace('module.', ''): v.detach().cpu()
...
def load_model(path='model.th', device='cpu'):
    estado = torch.load(path, map_location='cpu', weights_only=False)
    meta = estado['meta']
    ...
    if 'encoder_parcial' in estado:
        faltan, sobran = model.encoder.load_state_dict(estado['encoder_parcial'],
                                                       strict=False)
        if sobran:
            raise RuntimeError('claves desconocidas en el encoder: %s' % sobran[:3])
    elif 'encoder' in estado:
